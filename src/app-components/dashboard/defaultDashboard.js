...
import {
  Currentlysale,
  Marketvalue,
} from "../../components/dashboard/chartsData/apex-charts-data";
import {
  smallchart1data,
  smallchart1option,
  smallchart2data,
...
class defaultDashboard extends React.Component {
  ...
  componentDidMount = () => {
    ...
    if (curHr < 12) {
      ...
      this.setState({ daytimes: "Good Morning" });
    } else if (curHr < 18) {
      this.setState({ daytimes: "Good Afternoon" });
      //   setDayTimes("Good Afternoon");
    } else {
      this.setState({ daytimes: "Good Evening" });
      //   setDayTimes("Good Evening");
    }

    if (curHr >= 12) {
      //   setMeridiem("PM");
      this.setState({ curMi: "PM" });
    } else {
      //   setMeridiem("AM");
      this.setState({ curMi: "AM" });
    }
  ...
  render() {
    ...
    //   setDate(date);
    // };
    const { curHr, curMi, daytimes } = this.state;
    return (
      <Fragment>
                  ...
                  <div className="media">
                    <div className="badge-groups w-100">
                      <div className="badge f-12">
                        ...
                        />
                        <span id="txt">
                          {curHr}:{curMi < 10 ? "0" + curMi : curMi} {curMi}
                        </span>
                      </div>
                      <div className="badge f-12">
